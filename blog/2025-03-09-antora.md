...
authors: [ivanwalsh]
tags: [publishing, technical-writing]
...
For example, this site is run on Docusaurus, which is pretty easy to set up and maintain. It also works very nicely with Git and our web hosting provider, so for a relatively simple site, it's fine. I should add that Docusaurus is not just for lightweight document sites as it has quite a lot of rich features, themes, and a very active community. So, worth a peek if you're looking to get started. 
...
Antora is a static site generator designed specifically for technical documentation. Created by the team behind AsciiDoc, it treats your document set as a collection of 'versioned content sources' rather than a traditional website structure. This allows you to set up "documentation sites" that can aggregate content from multiple repos, branches, and versions into a single website.
...
Once set up, you can maintain documentation for multiple product versions simultaneously. This allows you to pull content from different Git branches/repositories and publish them as separate versions in a single UI. 
...
For tech docs teams that require cross-referencing between different products and versions, you can use its component-based architecture to coordinate your content workflows. 
...
If you're running a relatively small tech document team with a single product, I'd suggest looking at tools, such as Docusaurus, Hugo, Sphinx, and Github Pages. This will allow you to get started with publishing your tech docs. 

Once comfortable with the process, you can then look at more complex workflows. 

In the coming weeks, I'll go into more depth about how to set up a tech document website, tool usage, metrics and common pitfalls to avoid.
